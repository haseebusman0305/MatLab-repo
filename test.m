...
function K = K_Function(X1, X2, hp)
    % This function calculates the covariance matrix using the specified kernel
    % X1, X2: Input data points
    % hp: Hyperparameters (structure with fields L, sigma_p, kerneltype)

    % Calculate the squared Euclidean distance
    sqdist = pdist2(X1, X2, 'euclidean').^2;

    % Calculate the covariance matrix using the RBF kernel
    K = (hp.sigma_p^2) * exp(-0.5 * sqdist / (hp.L^2));
end

function x = CholeskySolve(L, b)
    % This function solves the linear system L*L'*x = b using Cholesky decomposition
...
function LML = calcLML(L, Y, nnum)
    % This function calculates the log marginal likelihood
    % L: Lower triangular matrix from Cholesky decomposition
    % Y: Training data outputs
    % nnum: Number of training data points

    % Calculate log determinant of V
    logDetV = 2 * sum(log(diag(L)));

    % Calculate LML
    LML = -0.5 * (Y' * (L' \ (L \ Y)) + logDetV + nnum * log(2 * pi));
end